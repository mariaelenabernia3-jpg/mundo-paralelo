document.addEventListener('DOMContentLoaded', () => {
    ...
    const ASSET_SOURCES = {
        ...
        ground: 'suelo.png',
        decor1: '1.png', decor2: '2.png', decor3: '3.png',
        decor4: '4.png', decor5: '5.png', decor6: '6.png'
    };
    const assets = {}; // This will hold the loaded Image objects
    ...
    const IDLE_FRAME = 0;
    const JUMP_FRAME = 1; 
    const WALK_CYCLE_START_FRAME = 2;
    const TOTAL_WALK_FRAMES = 4;

    // --- Game Objects (declarados aquí, definidos después de la carga) ---
    let player, camera, platforms, decorations;

    // --- Asset Loader Function (Solución al problema de la pantalla negra) ---
    function loadAssets(callback) {
        ...
        for (const key in ASSET_SOURCES) {
            ...
            img.onerror = () => {
                console.error(`Error al cargar: ${ASSET_SOURCES[key]}`);
                loadedCount++; // Aún se incrementa para no detener el juego
                if (loadedCount === totalAssets) {
                    callback();
                }
            };
    ...
    // --- Función para inicializar el juego después de cargar los assets ---
    function initializeGame() {
        ...
        decorations = [
            { x: 495, y: 350 - DECORATION_SIZE, assetKey: 'decor1' },
            { x: 600, y: 590 - DECORATION_SIZE, assetKey: 'decor2' },
            { x: 720, y: 450 - DECORATION_SIZE, assetKey: 'decor3' },
            { x: 900, y: 590 - DECORATION_SIZE, assetKey: 'decor4' },
            { x: 1250, y: 300 - DECORATION_SIZE, assetKey: 'decor5' },
            { x: 1620, y: 350 - DECORATION_SIZE, assetKey: 'decor6' },
        ];
    ...
    function update() {
        ...
        let moveX = 0;
        if (keys.ArrowLeft) {
            moveX = -MOVE_SPEED; player.facingDirection = 'left'; isMoving = true;
        }
        if (keys.ArrowRight) {
            moveX = MOVE_SPEED; player.facingDirection = 'right'; isMoving = true;
        }
        player.x += moveX;
        ...
        if (player.frameTimer > FRAME_SPEED) {
            player.frameTimer = 0;
            if (player.isJumping) {
                player.currentFrame = JUMP_FRAME;
            } else if (isMoving) {
                ...
                player.currentFrame = IDLE_FRAME;
            }
        }
    ...
    function draw() {
        ...
        for (const decor of decorations) {
            ...
            if (img) {
                ctx.drawImage(img, decor.x, decor.y, DECORATION_SIZE, DECORATION_SIZE);
            }
        }

        ctx.save();
        const drawY = player.y + PLAYER_VISUAL_OFFSET_Y;
        const sx = player.currentFrame * FRAME_WIDTH;
        const sy = 0;
    ...
    // --- Iniciar la carga de assets y luego el juego ---
